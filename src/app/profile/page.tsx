        ...
        <div className="max-w-4xl mx-auto px-4 md:px-8 py-6 space-y-0">
          ...
          <div className="relative">
            ...
            <div className="absolute -bottom-12 left-6 md:left-10">
              <div className={`w-24 h-24 md:w-28 md:h-28 rounded-2xl border-4 border-white dark:border-[#0A0A0A] shadow-xl flex items-center justify-center text-4xl font-black text-white bg-gradient-to-br ${gradientClass}`}>
                {initials}
              </div>
            </div>
            ...
            <div className="absolute -bottom-12 right-0 flex items-center gap-2">
              <Link
                ...
                className="flex items-center gap-1.5 px-4 py-2 text-sm font-bold text-slate-600 dark:text-slate-300 bg-white dark:bg-[#1A1A1A] border border-black/5 dark:border-white/10 rounded-xl hover:bg-slate-50 dark:hover:bg-white/5 transition-all shadow-sm"
              >
          ...
          <div className="pt-16 pb-6 px-0">
            <div className="flex flex-col gap-2">
              <div className="flex items-center gap-3 flex-wrap">
                <h1 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight">{user?.name || "Student User"}</h1>
                <span className="px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider bg-primary/10 text-primary rounded-full border border-primary/20">Student</span>
                {stats.streak > 0 && (
                  <span className="px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider bg-orange-50 dark:bg-orange-500/10 text-orange-500 rounded-full border border-orange-200 dark:border-orange-500/20">
                    🔥 {stats.streak}-day streak
                  </span>
                )}
              </div>
              <p className="text-sm text-slate-500 dark:text-slate-400">{user?.email} · Member since {memberSince}</p>
              ...
              {isEditingBio ? (
                <div className="space-y-2 mt-1">
                  <textarea
                    ...
                    rows={2}
                    maxLength={150}
                    className="w-full max-w-lg text-sm text-slate-700 dark:text-slate-300 bg-slate-50 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-xl px-3 py-2 outline-none resize-none focus:ring-2 focus:ring-primary/20"
                  ...
                  <input
                    ...
                    className="w-full max-w-lg text-sm text-slate-700 dark:text-slate-300 bg-slate-50 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-xl px-3 py-2 outline-none focus:ring-2 focus:ring-primary/20"
                  />
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={() => setIsEditingBio(false)} className="px-4 py-1.5 text-xs font-bold text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors">
                      Cancel
                    </button>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-2 mt-1 group/bio">
                  <div>
                    <p className="text-sm text-slate-600 dark:text-slate-400 max-w-lg">{bio || "No bio yet."}</p>
                    {website && (
                  ...
                  <button onClick={() => setIsEditingBio(true)} className="opacity-0 group-hover/bio:opacity-100 transition-opacity p-1 rounded-md hover:bg-slate-100 dark:hover:bg-white/10 text-slate-400 hover:text-slate-700 dark:hover:text-white flex-shrink-0 mt-0.5">
          ...
          {loading ? (
            <div className="grid grid-cols-3 md:grid-cols-6 gap-3 animate-pulse">
              {[1, 2, 3, 4, 5, 6].map(i => <div key={i} className="h-20 bg-slate-200 dark:bg-slate-700 rounded-2xl" />)}
            </div>
          ) : (
            <div className="grid grid-cols-3 md:grid-cols-6 gap-3">
              {[
                ...
                <div key={s.label} className="bg-white dark:bg-[#141414] rounded-2xl p-3 md:p-4 text-center border border-black/5 dark:border-white/10 shadow-sm hover:shadow-md transition-shadow">
                  ...
                  <p className="text-[9px] md:text-[10px] text-slate-400 dark:text-slate-500 font-bold uppercase tracking-wider mt-0.5">{s.label}</p>
                </div>
              ))}
            </div>
          )}
          ...
          {/* XP Progress */}
          <div className="bg-white dark:bg-[#141414] rounded-2xl border border-black/5 dark:border-white/10 shadow-sm p-5 mt-6">
            <div className="flex items-center justify-between mb-3">
              <div>
                <h3 className="text-sm font-bold text-slate-900 dark:text-white">Level Progress</h3>
          ...
          {/* Tabs */}
          <div className="flex items-center gap-1 border-b border-black/5 dark:border-white/10 pt-6 mt-2">
            {[
          ...
          <div className="pt-4 pb-16">
            {loading ? (
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                {[1, 2, 3, 4, 5, 6].map(i => <div key={i} className="aspect-square bg-slate-200 dark:bg-slate-700 rounded-2xl animate-pulse" />)}
              </div>
            ) : (activeTab === "courses" ? allCourses : publishedCourses).length === 0 ? (
              <div className="text-center py-20 bg-white dark:bg-[#141414] rounded-2xl border border-black/5 dark:border-white/10">
                <span className="material-symbols-outlined text-6xl text-slate-200 dark:text-slate-600 block mb-4">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                {(activeTab === "courses" ? allCourses : publishedCourses).map((course) => (
                  <Link
                    ...
                    className="group relative bg-white dark:bg-[#141414] rounded-2xl border border-black/5 dark:border-white/10 shadow-sm hover:shadow-lg transition-all overflow-hidden flex flex-col"
                  >
                    <div className={`h-1.5 ${
                    ...
                    <div className="p-4 flex flex-col gap-2 flex-1">
                      ...
                      <h4 className="text-sm font-bold text-slate-900 dark:text-white leading-snug line-clamp-2 group-hover:text-primary transition-colors">
                        {course.title}
                      </h4>
                      {course.description && (
                        <p className="text-[11px] text-slate-400 dark:text-slate-500 line-clamp-2 hidden md:block">{course.description}</p>
                      )}
                      <div className="mt-auto pt-2 border-t border-black/5 dark:border-white/10 flex items-center justify-between gap-2">
                    ...
                    <div className="absolute top-3 right-3 w-6 h-6 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
