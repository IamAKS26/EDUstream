      ...
      <div className="p-4 md:p-8 max-w-4xl mx-auto w-full">
        ...
        <div className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-bold font-display tracking-tight text-slate-900 dark:text-white mb-1">Local Device Notes</h1>
              <p className="text-sm text-slate-500 dark:text-slate-400">{notes.length} note{notes.length !== 1 ? "s" : ""} saved privately on this browser</p>
            </div>
          </div>
          <button
        ...
        {!loading && notes.length === 0 && (
          <div className="bg-white dark:bg-[#141414] rounded-2xl p-16 text-center border border-dashed border-black/10 dark:border-white/10 shadow-sm">
            <span className="material-symbols-outlined text-6xl text-slate-400 dark:text-slate-600 block mb-6 animate-bounce opacity-50">note_stack_add</span>
            <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-3">Your notebook is empty</h2>
            <p className="text-base text-slate-500 dark:text-slate-400 max-w-sm mx-auto leading-relaxed">
              Open any lesson, knowledge check, or video module and click the floating <strong className="text-primary font-bold mx-1">✏️ icon</strong> to start saving notes locally.
            </p>
          </div>
        )}
            ...
            <div key={moduleId} className="animate-in slide-in-from-bottom-4 fade-in">
              <div className="flex items-center gap-3 mb-4 pl-1">
                ...
                <h2 className="text-sm font-bold text-slate-900 dark:text-white uppercase tracking-wider">
                  {moduleId === "global" ? "General Notes" : (
                    <>Module <span className="text-slate-500 dark:text-slate-400 font-mono lowercase tracking-normal bg-slate-50 dark:bg-white/5 py-1 px-2 rounded-md ml-2 border border-black/5 dark:border-white/10">{moduleId.slice(-8)}</span></>
                  )}
                </h2>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {moduleNotes.map(note => (
                  <div key={note._id} className="bg-white dark:bg-[#141414] rounded-xl p-5 border border-black/5 dark:border-white/10 hover:border-primary/20 transition-all shadow-sm group flex flex-col h-full">
                    {editingId === note._id ? (
                      <div className="flex flex-col h-full flex-1">
                        <div className="flex items-center justify-between mb-3 border-b border-black/5 dark:border-white/10 pb-2">
                          ...
                          <button onClick={() => setEditingId(null)} className="text-xs font-bold text-slate-900 dark:text-white bg-slate-100 dark:bg-white/10 hover:bg-slate-200 dark:hover:bg-white/20 px-3 py-1.5 rounded-lg transition-colors">Done</button>
                        </div>
                        <textarea
                          autoFocus
                          value={draftContent}
                          onChange={e => handleEditChange(e.target.value)}
                          className="w-full h-full min-h-[120px] bg-slate-50 dark:bg-white/5 border border-black/5 dark:border-white/10 rounded-xl p-4 text-sm text-slate-900 dark:text-white outline-none resize-none focus:border-primary/40 focus:bg-white dark:focus:bg-white/10 transition-all leading-relaxed placeholder:text-slate-400"
                          placeholder="Write something brilliant..."
                        />
                      </div>
                    ) : (
                      <div className="flex flex-col h-full flex-1 cursor-text" onClick={() => startEdit(note)}>
                        <p className="text-sm text-slate-700 dark:text-slate-300 leading-relaxed flex-1 whitespace-pre-wrap">{note.content}</p>
                        <div className="flex items-end justify-between mt-4 pt-4 border-t border-black/5 dark:border-white/10 opacity-60 group-hover:opacity-100 transition-opacity">
                          <p className="text-[10px] text-slate-500 dark:text-slate-500 font-mono tracking-widest uppercase flex items-center gap-1">
