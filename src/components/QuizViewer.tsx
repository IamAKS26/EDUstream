  ...
  return (
    <div className="glass-card rounded-2xl p-8 max-w-3xl mx-auto">
      ...
      <div className="mb-8 flex justify-between items-center border-b border-black/5 dark:border-white/5 pb-4">
        <div>
          <h2 className="text-3xl font-bold text-slate-900 dark:text-white">{unit.title || "Knowledge Check"}</h2>
      ...
      <div className="space-y-8">
        {questions.map((q, idx) => (
          <div key={idx} className="p-6 bg-slate-50 dark:bg-slate-800/30 rounded-xl border border-black/5 dark:border-white/5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {q.options.map((opt) => {
                ...
                if (submitted) {
                  if (isCorrect) btnClass += "bg-green-500/20 border-green-500/50 text-green-700 dark:text-green-300 shadow-[0_0_10px_rgba(34,197,94,0.2)]";
                  else if (isWrongSelected) btnClass += "bg-red-500/20 border-red-500/50 text-red-600 dark:text-red-300";
                  else btnClass += "bg-slate-50 dark:bg-slate-800/50 border-black/5 dark:border-white/5 text-slate-500 dark:text-slate-500 opacity-50";
                } else {
                  if (isSelected) btnClass += "bg-amber-50 dark:bg-accent/20 border-amber-400 dark:border-accent/50 text-amber-800 dark:text-accent shadow-[0_0_10px_rgba(255,215,0,0.15)]";
                  else btnClass += "bg-white dark:bg-slate-800/30 border-black/8 dark:border-white/5 text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700/40 hover:border-slate-300 dark:hover:border-white/20";
                }
      ...
      <div className="mt-10 flex justify-end pt-6 border-t border-black/5">
        {!submitted ? (
          ...
          <div className="flex items-center gap-6 w-full justify-between">
            <div className="text-xl font-bold bg-slate-50 dark:bg-slate-800/50 px-4 py-2 rounded-lg border border-black/5 dark:border-white/10 flex items-center gap-2 text-slate-900 dark:text-white">
              Score:
              <span className={scorePercent >= 60 ? "text-green-600 dark:text-green-400" : "text-red-500 dark:text-red-400"}>
                {scorePercent}%
              </span>
              {scorePercent >= 60
                ? <span className="material-symbols-outlined text-green-600 dark:text-green-400 text-lg">verified</span>
